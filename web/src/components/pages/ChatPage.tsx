...
function ChatPlaceholder() {
  ...
  const activeAgent = resolvedAgentId
    ? agents.find((agent) => agent.id === resolvedAgentId || agent.name === resolvedAgentId)
    : null;
      ...
      <div className="flex items-start gap-3 text-left">
        {activeAgent ? (
          ...
          <div className="size-9 shrink-0 rounded-lg bg-foreground/4 border border-border/50 flex items-center justify-center mt-0.5">
            <HashIcon className="size-4 text-foreground/45" />
          </div>
        )}
        <div className="flex flex-col gap-1 min-w-0">
          <h1 className="text-[15px] font-semibold tracking-tight text-foreground">{title}</h1>
          <p className="text-[12px] text-muted-foreground/75 leading-snug">{subtitle}</p>
        </div>
      </div>
