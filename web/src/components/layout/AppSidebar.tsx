    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto px-2.5 py-2">
        {defaultAgent && (
          <div className="mb-6">
            <button
              onClick={() => onNavigate(`/?conversationId=dm_${defaultAgent.id || defaultAgent.name}`)}
              className={cn(
                "flex items-center justify-start gap-2.5 w-full px-2.5 h-8 rounded-md text-[13px] text-left transition-all duration-150",
                conversationId === `dm_${defaultAgent.id || defaultAgent.name}` && currentTab === "chat"
                  ? "bg-muted/60 text-foreground font-medium"
                  : "text-muted-foreground hover:bg-muted/40 hover:text-foreground"
              )}
            >
              <span className="w-3 shrink-0 flex items-center justify-center">
                {conversationsActivity?.byConversation?.[`dm_${defaultAgent.id || defaultAgent.name}`]?.active && (
