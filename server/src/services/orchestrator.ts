...
import { memoryPlugin, memoryToolDefinitions, createMemoryPromptBuilder } from "../plugins/memory.js";
...
export function orchestrationToolsPlugin(options: {
  ...
  return (builder: MelonyBuilder<ConversationState, ConversationEvent>) => {
    builder.on("action:updateSessionState", async function* (event: ConversationEvent, context: { runId: string; state: ConversationState }) {
      ...
      try {
        ...
        yield {
          type: "action:result",
          data: { action: "updateSessionState", result: `Successfully updated state at path "${path}".`, toolCallId },
        } as ConversationEvent;
      } catch (error: any) {
        yield {
          type: "action:result",
          data: { action: "updateSessionState", result: `Error updating state: ${error.message}`, toolCallId },
        } as ConversationEvent;
      }
    });

    builder.on("action:delegateTask", async function* (event: ConversationEvent, context: { runId: string; state: ConversationState }) {
      ...
      if (!agentRuntime) {
        yield {
          type: "action:result",
          data: { action: "delegateTask", result: `Error: Agent "${agentId}" not found.`, toolCallId },
        };
        return;
      }
      ...
      const agentIterator = agentRuntime.run(
        { type: "agent:input", data: { content: task, attachments }, meta: { threadId: delegationId } } as any,
      ...
      try {
        for await (const agentEvent of agentIterator) {
          if (agentEvent.type === "suspend") {
            ...
            yield { ...agentEvent, meta: { ...agentEvent.meta, delegationId, agentName: agentId, threadId: delegationId } } as ConversationEvent;
            continue;
          }

          if (agentEvent.type === "agent:input") {
            yield { ...agentEvent, type: "agent:sub-input", meta: { ...agentEvent.meta, delegationId, agentName: agentId, threadId: delegationId } } as ConversationEvent;
            continue;
          }

          if (agentEvent.type.startsWith("action:") && agentEvent.type !== "action:result") {
            yield { ...agentEvent, type: "agent:sub-action", meta: { ...agentEvent.meta, delegationId, agentName: agentId, threadId: delegationId }, data: { ...agentEvent.data, originalType: agentEvent.type } } as ConversationEvent;
            continue;
          }

          if (agentEvent.type === "action:result") {
            yield { ...agentEvent, type: "agent:sub-action-result", meta: { ...agentEvent.meta, delegationId, agentName: agentId, threadId: delegationId } } as ConversationEvent;
            continue;
          }

          if (agentEvent.type === "usage:update") {
            yield { ...agentEvent, type: "agent:sub-usage", meta: { ...agentEvent.meta, delegationId, agentName: agentId, threadId: delegationId } } as ConversationEvent;
            continue;
          }
      ...
      if (pendingApprovalId) {
        ...
        state.pendingAgentTasks[pendingApprovalId] = { toolCallId, agentName: agentId, delegatorAgentId, delegationId, stateKey: typeof stateKey === "string" ? stateKey : undefined };
        return;
      }
      ...
      yield {
        type: "action:result",
        data: { action: "delegateTask", result: lastAgentOutput || "Task completed with no output.", toolCallId },
      } as ConversationEvent;
    });
  };
}
...
export function llmOrchestratorPlugin(options: {
  ...
  const { model, resolvedModelId, resolvedBaseDir, registry, system, toolDefinitions = {}, outputSchema } = options;
