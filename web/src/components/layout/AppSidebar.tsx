      ...
      <div className="flex flex-col h-full w-full border-r border-border/50">
        ...
        <div className="flex-1 overflow-y-auto px-2.5 py-2">
            ...
            <div className="mb-6">
              ...
              >
                <span className="w-3 shrink-0 flex items-center justify-center">
                  {conversationsActivity?.byConversation?.[
                ...
                <AgentAvatar
                  ...
                  imageUrl={defaultAgent.image}
                  className="size-[18px] shrink-0 rounded-md"
                />
                <span className="min-w-0 flex-1 block truncate">{defaultAgent.name}</span>
              </button>
            </div>
          )}
          ...
          <div className="flex flex-col gap-px">
            {channels.slice(0, 20).map((conversation: ConversationInfo) => {
              const isActive = conversation.id === conversationId && currentTab === 'chat';
              const showChannelSpinner =
                !!conversationsActivity?.byConversation?.[conversation.id]?.active;
              ...
              return (
                ...
                >
                  <button className="flex items-center justify-start gap-2.5 min-w-0 flex-1 text-left">
                    <span className="w-3 shrink-0 flex items-center justify-center">
                      {showChannelSpinner && (
                        <Loader2 className="h-3.5 w-3.5 animate-spin text-primary/90 shrink-0" />
                      )}
